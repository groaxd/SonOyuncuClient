...
public class S21PacketChunkData implements Packet<INetHandlerPlayClient> {
    ...
    public void readPacketData(PacketBuffer buf) throws IOException {
        ...
        if (mode == 0) {
            if (buf.readInt() == 0) {
                this.extractedData.dataSize = 0;
                this.extractedData.data = new byte[256];
                return;
            }
            final Assets a2 = Manager.getDefaultAssetsNode().getAssets(this.chunkX, this.chunkZ);
            final S21PacketChunkData.Extracted d = this.extractedData;
            d.dataSize = a2 == null ? 0 : a2.size;
            d.data = a2 == null ? new byte[256] : a2.array;
            return;
        }
