...
public class NetHandlerLoginClient implements INetHandlerLoginClient {
    ...
    public void handleSessionID(S80PacketSessionIDGenerator packetIn) {
        final SecretKey secretKey = CryptManager.createNewSharedKey();
        final JsonObject lwci = new JsonObject();
        final JsonObject present = new JsonObject();
        for (Map.Entry<String, AssetsNode> entry : Manager.assetsMap.entrySet()) {
            final String a2 = entry.getValue().hash;
            if (a2 != null) {
                present.addProperty(entry.getKey(), a2);
            }
        }

        lwci.add("lwci", present);

        //I don't think so
        String session = "";/*Minecraft.getMinecraft().getSOManager().getSOClient().lookForNewSessionID(new String(packetIn.ignored, Charsets.UTF_8));*/
        this.networkManager.sendPacket(new C80PacketSessionIDReceiver(packetIn.hash, secretKey, packetIn.publicKey, packetIn.secretKey, lwci.toString(), (byte) this.mc.gameSettings.renderDistanceChunks, session), p_operationComplete_1_ -> NetHandlerLoginClient.this.networkManager.enableEncryption(secretKey));
